...
pt[1] -> FixChecksums                        		// recalculate checksum
	 -> gs :: IPClassifier(dest 192.168.42.5, -)  	// classify local network and external network traffic
	 					// set gateway annotation to dst addr in local network
	 -> [0]gate_selector[0]
         -> Print(AfterGWS)
         -> Queue 
         -> [0]rrs
...
apcl[2] -> ARPResponder($AP_IP_ADDR $AP_MAC_ADDR) 
	-> Queue
        -> [1]rrs2 
...
fd_cl[3] -> Print(GOTmacPing) -> [1]gate_selector;
